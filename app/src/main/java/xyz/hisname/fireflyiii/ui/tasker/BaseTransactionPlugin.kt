...
abstract class BaseTransactionPlugin: AbstractAppCompatPluginActivity() {
    ...
    private val resultBlurb by lazy { StringBuilder().append("The following will be added: ") }
    ...
    override fun finish() {
        ...
        if (TaskerPlugin.hostSupportsRelevantVariables(intent.extras)) {
            ...
            resultIntent.putExtra(com.twofortyfouram.locale.api.Intent.EXTRA_STRING_BLURB, resultBlurb.toString())
            TaskerPlugin.addRelevantVariableList(resultIntent, arrayOf("%response\nOutput\nThe <B>response</B> in JSON",
            "%errmsg\nError\nThe <B>error message</B>"))
        }
