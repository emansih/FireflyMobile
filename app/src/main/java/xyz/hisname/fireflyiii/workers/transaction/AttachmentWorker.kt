...
class AttachmentWorker(private val context: Context, workerParameters: WorkerParameters): BaseWorker(context, workerParameters)  {
    ...
    override fun doWork(): Result {
        ...
        val requestFile = RequestBody.create(MediaType.parse("multipart/form-data"), fileObject)
        ...
        service?.storeAttachment(fileName, "Transaction", transactionId, fileName,
            ...
            if (response.code() == 200 && responseBody != null) {
                service.uploadFile(responseBody.data.id, requestFile, fileToUpload).enqueue(retrofitCallback({ uploadFileResponse ->
